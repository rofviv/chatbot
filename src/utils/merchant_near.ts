...
export default class Utils {
  static async merchantNear(merchants: MerchantModel[], latitude: number, longitude: number): Promise<MerchantModel[]> {
    const merchantsNear = merchants.filter((merchant) => {
      const distance = this.calculateDistance(latitude, longitude, merchant.latitude, merchant.longitude);
